          ...
          {weddings.map((w) => {
            ...
            return (
              <li key={w.id}>
                <Link
                  href={`/admin/${w.id}`}
                  className="flex items-center gap-4 rounded-2xl border border-line bg-surface p-4 transition-colors hover:border-rose/50"
                >
                  <span
                    className={`h-2.5 w-2.5 flex-none rounded-full ${
                      live ? "bg-rose" : night.revealed ? "bg-go" : "bg-paper-3/40"
                    }`}
                    aria-hidden
                  />
                  ...
                  <span className="tabular flex-none text-right text-[13px] text-paper-2">
                    <span className="block text-gold">{w._count.frames} frames</span>
